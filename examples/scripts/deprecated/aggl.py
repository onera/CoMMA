...
# set the matrix (WARNING: The vector is started from 1 because of a bug or in the
# writing of the data or in the interface CoMMA/CODA FIXME: TO BE CHECKED
sp = csr_matrix(
    (
        a[" \nadjMatrix_areaValues"][1:],
        a[" \nadjMatrix_col_ind"][1:],
        a["adjMatrix_row_ptr"][1:],
    )
)
# Convert to the Adjacency matrix for Networkx
G = nx.from_scipy_sparse_matrix(sp, parallel_edges=True, create_using=nx.MultiGraph)
