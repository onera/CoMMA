...
unordered_map<long, long> Dual_Graph::compute_fc_compactness_inside_a_cc(unordered_set<long> set_of_fc) {
  /*
    :param set_of_fc:
    :return:
  */
  unordered_map<long, long> dict_fc_compactness;
  if (set_of_fc.size() > 1) {
    // for every fc constituting a cc
    for(auto i_fc : set_of_fc) {
      for(auto ind_fc_neighbour : get_neighbours(i_fc)) {
	if(set_of_fc.count(ind_fc_neighbour) > 0 && ind_fc_neighbour != i_fc) {
...
double Dual_Graph::compute_average_fc_compactness_inside_a_cc(unordered_set<long> set_of_fc) {
  // TODO Delete unused???
  if(set_of_fc.size() > 1) {
    unordered_map<long,long> dict_fc_compactness = compute_fc_compactness_inside_a_cc(set_of_fc);
...
void Dual_Graph::compute_breadth_first_search(unordered_set<long> set_of_fc, long current_seed, unordered_map<long,long> dict_inv_list_of_fc, vector<long> color, long & max_color, unordered_set<long> & s_fc_max_color, unordered_map<long, vector<long>> & d_spanning_tree) {
  /*
    :param set_of_fc:
    :param current_seed:
