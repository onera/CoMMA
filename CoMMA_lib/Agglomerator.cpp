...
Agglomerator::Agglomerator(Dual_Graph<long,double> &graph, Coarse_Cell_Graph &cc_graph,
                           short dimension)
...
// =======================
Agglomerator_Anisotropic::Agglomerator_Anisotropic(Dual_Graph<long,double> &graph,
  ...
  _v_of_s_anisotropic_compliant_fc[0] =
      _fc_graph._s_anisotropic_compliant_cells;
...
void Agglomerator_Anisotropic::get_agglo_lines(
    int level, vector<long> &agglo_lines_array_idx,
...
void Agglomerator_Anisotropic::agglomerate_one_level(const short goal_card,
  ...
  if (_v_lines[0].empty()) {
    ...
    _v_lines[0] = _fc_graph.compute_anisotropic_line(
...
void
Agglomerator_Anisotropic::create_all_anisotropic_cc_wrt_agglomeration_lines() {
  ...
  for (auto fLIt = _v_lines[0].begin(); fLIt != _v_lines[0].end(); fLIt++) {
    ...
    // level 0.
    // We create a pointer to an empty deque for the line + 1, and hence for the
    ...
    if (actual_deque.size() <= 1) {
      // the agglomeration_line is empty and hence the iterator points again to
      // the
    ...
    for (auto deqIt = actual_deque.rbegin(); deqIt != actual_deque.rend();
      ...
      // cardinal 2
      // ANISOTROPIC: we agglomerate 2 by 2, hence the anisotropic agglomeration
      // will
      // provoke agglomerated coarse cells of cardinality 2.
      // Here we have to consider the two different case in which we have an odd
      // number of cells.
...
void Agglomerator_Isotropic::set_agglomeration_parameter(short goal_card,
                                                         short min_card,
                                                         short max_card) {
...
Agglomerator_Biconnected::Agglomerator_Biconnected(Dual_Graph<long,double> &graph,
                                                   Coarse_Cell_Graph &cc_graph,
                                                   short dimension)
...
void Agglomerator_Isotropic::agglomerate_one_level(const short goal_card,
  ...
  while (_cc_graph->get_number_of_fc_agglomerated() < nb_of_fc) {
    // 1) Choose a new seed
    long seed = (_fc_graph._seeds_pool)
                    .choose_new_seed(_cc_graph->_a_is_fc_agglomerated);
    ...
    unordered_set<long> set_current_cc =
    ...
    _cc_graph->cc_create_a_cc(set_current_cc, is_anistropic,
                              is_creation_delayed);
  }
...
unordered_set<long>
Agglomerator_Biconnected::choose_optimal_cc_and_update_seed_pool(
    const long seed, short &compactness) {
  bool is_order_primary = false;
  bool increase_neighbouring = true;
  //  The goal of this function is to choose from a pool of neighbour the better
  ...
  // Dictionary of the neighborhood of the seed, the key is the global index of
  ...
  _fc_graph.compute_neighbourhood_of_cc(
      ...
      _max_card, _cc_graph->_a_is_fc_agglomerated);  // anisotropic cells
                                                     // agglomerated (not to
  ...
  // The available neighborhood cells are not enought to reach the goal
  // cardinality
    ...
    if (is_creation_delayed) {
      ...
      compactness = _dimension;  // minimum number of nrighborhood of a
    ...
    // This dictionary is used to store the eligible cc: i.e. its size is inside
    // the permitted range.
    // This is useful to track back our step if needed.
    // [size of the current, [cell set, d_n_of seed]]
    unordered_map<short, pair<unordered_set<long>, unordered_map<long, int>>>
        dict_cc_in_creation;
    double min_external_faces = numeric_limits<double>::max();
    short arg_min_external_faces = min_size;
    // Here we define the exact dimension of the coarse cell as the min between
    // the max cardinality given as an input
    // (remember the constructor choice in case of -1) and the dictionary of the
    // boundary cells, it means the total number of
    // neighborhood cells until the order we have given (as default 3, so until
    // the third order)
    ...
    int number_of_external_faces_current_cc =
        nb_neighbours + _fc_graph._seeds_pool.boundary_value(seed) - 1;
    // d_keys_to_set from Util.h, it takes the keys of the unordered map and
    // create an unordered set. The unordered
    // set is representing hence all the neighborood of seed until a given
    // order.
    unordered_set<long> s_neighbours_of_seed = d_keys_to_set(d_n_of_seed);
    // Build the class first order neighborhood
    First_Order_Neighbourhood f_o_neighbourhood =
        First_Order_Neighbourhood(s_neighbours_of_seed);
    ...
    while (size_current_cc < max_ind) {
      ...
      compute_best_fc_to_add(_fc_graph, fon, d_n_of_seed, is_order_primary,
                             cc_surf, vol_cc, s_current_cc,
                             argmin_ar,  // output
                             max_faces_in_common, min_ar_surf, min_ar_vol);

      number_of_external_faces_current_cc +=
          _fc_graph.get_nb_of_neighbours(argmin_ar) +
          _fc_graph._seeds_pool.boundary_value(argmin_ar) - 1 -
      ...
      // This choice is based on the idea that the smallest cc (w.r.t. card) is
      ...
      if ((min_size <= size_current_cc) || size_current_cc == max_ind) {
        ...
        pair<unordered_set<long>, unordered_map<long, int>> p =
  ...
  if (!d_n_of_seed.empty()) {
    ...
    // Reminder: d_n_of_seed is here the pool of cell neighbouring the previous
    // seed!
    for (auto &i_k_v : d_n_of_seed) {
      if (i_k_v.second <= 2) {
...
void Agglomerator_Biconnected::compute_best_fc_to_add(
    Dual_Graph<long,double> &graph, unordered_set<long> fon,  // First order neighborhood
  ...
  for (const long &i_fc :
    ...
    const short &order =
        d_n_of_seed.at(i_fc);  // [i_fc] is not const the method at returns the
                               // reference to the value of the key i_fc.

    // TODO This version seems good but refactorisation to do: perhaps it is not
    ...
    if (number_faces_in_common >=
      ...
      if (number_faces_in_common == max_faces_in_common or is_order_primary) {
        ...
        if (order <=
            d_n_of_seed.at(
                arg_max_faces_in_common)) {  // [arg_max_faces_in_common] is not
                                             // const.
