...
bool computeAnisotropicLine(long* sizes,
    ...
    long numberOfFineAnisotropicCompliantCells = sizes[7];
    long numberOfAnisotropicLinesPOne_size = sizes[8];  // numberOfFineAnisotropicCompliantCells at the  beginning
    long agglomerationLines_size = sizes[9];            // numberOfFineAnisotropicCompliantCells at the  beginning
...
//    cout<<"agglomerationLines_size "<<agglomerationLines_size<<endl;
    ...
    unordered_map<long, double> dictAnisotropicCell;// keys are the ratio Max to average (ratioArray[iCell]) and value
    ...
    long cellNumber;
    long ind, indPOne, indNeighborCell;
...
//    unordered_map<long, double>::iterator uMapIt;
...
//    }
    if (!dictAnisotropicCell.empty()) {
        ...
        multimap<double, long> sorted_anisotropicCells = flip_map(dictAnisotropicCell);  // Sorting!
        multimap<double, long>::reverse_iterator rIt;

        long numberOfAnisotropicCells = dictAnisotropicCell.size();
//        cout<<"numberOfAnisotropicCells "<<numberOfAnisotropicCells<<endl;
        long* indexLocalToGlobal = new long[numberOfAnisotropicCells];
        ...
        for (long iLoc =0; iLoc< numberOfAnisotropicCells; iLoc++)
        {
        ...
        long iGlobalSeed = indexLocalToGlobal[iLocalSeed];
...
void agglomerate_Anisotropic_One_Level_without_list_lines(long *sizes,
    ...
    long fineAgglomerationLines_array_Idx_size = sizes[0];
    long numberOfFineCells = sizes[1];
    long numberOfFineAgglomeratedCells = sizes[2];
    long indCoarseCell = sizes[3];
    long AnisotropicCompliantCoarseCells_size = sizes[4];

    set<long> setOfAnisotropicCompliantCoarseCells;
    ...
    long* new_Lines_array = new long[fineAgglomerationLines_array_size];

    long iCountCoarseCell = 0;
    long i_New_numberOfLines = 0;
...
//    cout<<"End of update"<<endl;
    ...
    if (!setOfAnisotropicCompliantCoarseCells.empty()){
        // array_AnisotropicCompliantCoarseCells = np.zeros((len(setOfAnisotropicCompliantCoarseCells),), dtype=long);
        set<long>::const_iterator sit  = setOfAnisotropicCompliantCoarseCells.begin ();
