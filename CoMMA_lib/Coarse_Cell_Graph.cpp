...
Coarse_Cell_Graph::Coarse_Cell_Graph(const Dual_Graph &fc_graph, int verbose)
...
long Coarse_Cell_Graph::cc_create_a_cc(const unordered_set<long> &s_fc,
  ...
  if (is_anisotropic) {
    ...
    // i_cc
    _cc_vec.insert(
        pair<long, shared_ptr<Subgraph>>(_cc_counter, new_cc->_cc_graph));
    is_mutable = false;
  }
...
map<long, shared_ptr<Subgraph>>::iterator Coarse_Cell_Graph::remove_cc(
