...
// print("cc_create_a_cc", s_fc, "is_creation_delayed", is_creation_delayed)
    ...
    if (is_anisotropic) {
        ...
        (*this)._d_anisotropic_cc[(*this)._cc_counter] = new unordered_set<long>(s_fc);  //TODO check there is a copy???
...
unordered_map<long, unordered_set<long>*>* Coarse_Cell_Graph::get_d_cc_all()
{
    ...
    unordered_map<long, unordered_set<long>*>* d= new unordered_map<long, unordered_set<long>*>();
...
//        (*d)[i_cc] = (*(* i_k_v.second).get_s_fc());
    ...
    for(auto & i_k_v : (*this)._d_anisotropic_cc)
    {
        long i_cc = i_k_v.first;
        (*d)[i_cc] = i_k_v.second;
    }
