...
class Graph {
 public:
  ...
  Graph(const CoMMAIndexType &nb_c, const vector<CoMMAIndexType> &m_crs_row_ptr,
        const vector<CoMMAIndexType> &m_crs_col_ind, const vector<CoMMAWeightType> &m_crs_values,
  ...
  /** @brief Based on the CRS representation retrieves the neighbours of the cell
   * given as an input.
   ...
   * input, we retrieve the faces connecting the given cell with the neighborhood
...
class Subgraph : public Graph<CoMMAIndexType,CoMMAWeightType> {
 public:
   ...
   *  @param[in] volumes The volumes of the cells
   */
  Subgraph(const CoMMAIndexType &nb_c, const vector<CoMMAIndexType> &m_crs_row_ptr,
           const vector<CoMMAIndexType> &m_crs_col_ind,
           const vector<CoMMAWeightType> &m_crs_values, const vector<CoMMAWeightType> &volumes,
           const vector<CoMMAIndexType> &mapping_l_to_g, const bool &is_isotropic)
  ...
  void insert_node(vector<CoMMAIndexType> &v_neigh, const CoMMAIndexType &i_fc,
                   const CoMMAWeightType &volume, const vector<CoMMAWeightType> &weight) {
    // Use the mapping
  ...
  void remove_node(const CoMMAIndexType &elemento) {
    ...
    // getting neighbours
    vector<CoMMAIndexType> v_neigh = get_neighbours(i_fc);
...
class Dual_Graph : public Graph <CoMMAIndexType,CoMMAWeightType> {
 ...
 public:
   ...
   */
  Dual_Graph(const CoMMAIndexType &nb_c, const vector<CoMMAIndexType> &m_crs_row_ptr,
             const vector<CoMMAIndexType> &m_crs_col_ind,
             const vector<CoMMAWeightType> &m_crs_values, const vector<CoMMAWeightType> &volumes,
  ...
  */
  vector<deque<CoMMAIndexType> *> compute_anisotropic_line(CoMMAIndexType &nb_agglomeration_lines);
  ...
  void compute_d_anisotropic_fc(vector<CoMMAWeightType> &maxArray,
    ...
    for (const CoMMAIndexType i_fc : _s_anisotropic_compliant_cells) {
      ...
      vector<CoMMAIndexType> v_neighbours = get_neighbours(i_fc);
      vector<CoMMAWeightType> v_weights = get_weights(i_fc);
 ...
 */
  ...
  */
  ...
  void compute_neighbourhood_of_cc(const unordered_set<CoMMAIndexType> s_seeds,
                                   int &nb_of_order_of_neighbourhood,
                                   unordered_map<CoMMAIndexType, int> &d_n_of_seed,
                                   const int max_card,
                                   vector<bool> &is_fc_agglomerated_tmp) {
    // Basic checks
    assert(max_card != -1);
    unordered_map<CoMMAIndexType, int> d_n_of_order_o_m_one;  // dict of FC with the order
    ...
    while ((i_order < nb_of_order_of_neighbourhood + 1) ||
      ...
      for (const auto &i_k_v : d_n_of_order_o_m_one) {
        ...
        for (const CoMMAIndexType &i_fc_n : get_neighbours(seed_tmp)) {
