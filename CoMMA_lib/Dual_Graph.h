...
class Dual_Graph : public Graph<CoMMAIndexType, CoMMAWeightType, CoMMAIntType> {
 ...
 public:
   ...
   *  @return a vector of pointers to the deque representing the lines
   */
  vector<deque<CoMMAIndexType> *> compute_anisotropic_line(
    ...
    compute_anisotropic_fc(maxArray, anisotropic_fc, threshold_anisotropy, 0);
    ...
    for (auto &i_fc : anisotropic_fc) {
      ...
      while (!end) {
        ...
        const vector<CoMMAIndexType> v_neighbours = this->get_neighbours(seed);
        const vector<CoMMAWeightType> v_w_neighbours = this->get_weights(seed);
