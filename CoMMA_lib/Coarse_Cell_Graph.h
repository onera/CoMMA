...
class Coarse_Cell_Graph {
 ...
 public:
  /** @brief Constructor*/
  Coarse_Cell_Graph(const Dual_Graph<CoMMAIndexType, CoMMAWeightType> &fc_graph): _fc_graph(fc_graph){
  ...
  inline CoMMAIndexType get_nb_of_cc() { return _cc_counter; }
  /** @brief map container of the CSR representation of the coarse cells */
  map<CoMMAIndexType, shared_ptr<Subgraph<CoMMAIndexType, CoMMAWeightType>>> _cc_vec;
   ...
   */
  vector<CoMMAIndexType> get_neigh_cc(const CoMMAIndexType &i_fc, const CoMMAIndexType &i_cc)
...
/** @brief Update the member variable of the memebr fc2cc based on the mapping given
 ...
 * @return iterator to the next element*/
  map<CoMMAIndexType, shared_ptr<Subgraph<CoMMAIndexType, CoMMAWeightType>>>::iterator remove_cc(
      map<CoMMAIndexType, shared_ptr<Subgraph<CoMMAIndexType, CoMMAWeightType>>>::iterator elim)
{
  // we delete the element and we obtainer the pointer to the next element in
  // memory
  map<CoMMAIndexType, shared_ptr<Subgraph<CoMMAIndexType, CoMMAWeightType>>>::iterator it =
      _cc_vec.erase(elim);
  // We get the length
 ...
 * correction of singular cells (if one cell is alone after the agglomeration step is agglomerated
 ...
 * @param[in] max_card Maximum cardinality of the agglomerator. */
  ...
  CoMMAIndexType cc_create_a_cc(const unordered_set<CoMMAIndexType> &s_fc,
...
{  // Create a course cell from the fine cells and update the fc_2_cc tree.
  ...
  if (is_anisotropic) {
    ...
    auto new_cc = make_shared<Coarse_Cell<CoMMAIndexType, CoMMAWeightType>>(_fc_graph, _cc_counter,
                                                         s_fc, false);
    // we collect the various cc_graph, where the index in the vector is the
    // i_cc
    _cc_vec.insert(pair<CoMMAIndexType, shared_ptr<Subgraph<CoMMAIndexType, CoMMAWeightType>>>(
  ...
  if (!is_creation_delayed) {
    ...
    if (is_mutable) {
      ...
      //==================
      auto new_cc =
          make_shared<Coarse_Cell<CoMMAIndexType, CoMMAWeightType>>(_fc_graph, _cc_counter, s_fc);
      // we collect the various cc_graph, where the index in the vector is the
      // i_cc
      _cc_vec.insert(pair<CoMMAIndexType, shared_ptr<Subgraph<CoMMAIndexType, CoMMAWeightType>>>(
          _cc_counter, new_cc->_cc_graph));
...
/** @brief Creates all the delayed coarse cell. It works only when the delayed cell flag
