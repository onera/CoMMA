...
class Seeds_Pool {
 ...
 public:
  ...
  CoMMAIndexType choose_new_seed(const vector<bool> &a_is_fc_agglomerated) {
    ...
    // with l--
    for (int i_l = CoMMACellT::CORNER; i_l >= CoMMACellT::INTERIOR; i_l--) {
  ...
  */
  int boundary_value(const CoMMAIndexType &i_fc) {
  ...
  void update(const list<CoMMAIndexType> &l_new_seeds,
    ...
    if (!is_extremal_vertex) {
      for (const CoMMAIndexType &i_new_seed : l_new_seeds) {
        ...
        // via Metis or Scotch.
        int value_is_on_bnd = CoMMACellT::INTERIOR;
  ...
  bool is_empty(int i_level = 0) {
    ...
    if (_l_of_seeds.size() > 0) {
      for (int i = CoMMACellT::CORNER; i > i_level -1; i--) {
  ...
  */
  ...
  unordered_map<CoMMAIndexType, int> _d_is_on_bnd;
};
