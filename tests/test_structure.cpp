...
SCENARIO("Test of the stack", "[stack]") {
 GIVEN("A simple stack of long type") {
    ...
    st_long.push(2); 
    THEN(
          "We can check that the operation on the stack are correctly realized") {
...
SCENARIO("Test of the seed pool", "[Seed Pool]") {
 GIVEN("The DualGPy configuration used for the example") {
   ...
   Seeds_Pool* sp = new Seeds_Pool(Data.nb_fc,
               Data.d_is_on_bnd,
               Data.s_is_on_corner,
               Data.s_is_on_ridge,
               Data.s_is_on_valley,
               2);
    THEN(
          "We see that the sequence of seed choice is the one of Diskin Mavriplis") {
    ...
    class test : public Seeds_Pool{
	    public:
		 test(int number_of_cells,
                       ...
                       int init_bnd_level): Seeds_Pool(number_of_cells,d_is_on_bnd,is_on_corner,is_on_ridge,is_on_valley,init_bnd_level){};
