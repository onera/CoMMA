...
GIVEN(
  ...
  WHEN("We agglomerate the mesh") {
    ...
    THEN(
      ...
      REQUIRE(
        seeds_pool->choose_new_seed(cc_graph->_is_fc_agglomerated).value() == 24
      );
    }
...
GIVEN(
  ...
  WHEN("We agglomerate the mesh") {
    ...
    THEN(
      ...
      REQUIRE(
        seeds_pool->choose_new_seed(cc_graph->_is_fc_agglomerated).value() == 18
      );
    }
